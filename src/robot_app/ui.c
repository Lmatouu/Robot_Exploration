...
static void handle_create_path() {
	/*TODO: prompt for the number of steps and create an empty path, print success or failure messages accordingly */
	/*HINT: update steps_number on success or set to 0 on failure */
  
  int nb_steps = handle_user_prompt_int(UI_ASK_SIZE_PATH, 0, 1000);
  if (copilot_create_path(nb_steps) == EXIT_SUCCESS) {
    print_success_message(CMD_CREATE_PATH);
    steps_number = nb_steps;
  } else {
